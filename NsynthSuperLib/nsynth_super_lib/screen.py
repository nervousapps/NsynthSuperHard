...
class Screen:
    def __init__(self):
        ...
        self.font = ImageFont.truetype(
            "/usr/share/fonts/truetype/liberation2/LiberationSans-BoldItalic.ttf", 12)
    ...
    def draw_text_box(self, text, clear=False):
        if clear:
            self.clear()
        image, draw = self._create_blank_image()
        ...
        draw.rectangle(
            (BORDER, BORDER, self.oled.width
             - BORDER - 1, self.oled.height - BORDER - 1),
            outline=0,
            fill=0,
        )
